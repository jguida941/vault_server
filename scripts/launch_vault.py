...
def check_node():
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"{GREEN}✓ Node.js found: {result.stdout.strip()}{RESET}")
            return True
    ...
    try:
        ...
        if result == 0:
            ...
            try:
                ...
                time.sleep(1)
                print(f"{GREEN}✓ Cleared port {port}{RESET}")
            except Exception as e:
                print(f"{RED}❌ Could not clear port: {e}{RESET}")
                return False
    except Exception:
        pass  # Port is free
...
def main():
    ...
    print(f"{YELLOW}🔥 THE VAULT 🔒 - YouTube Music Player{RESET}")
    print(f"{BLUE}{'='*50}{RESET}\n")
    ...
    if not check_port(8888):
        print(f"{RED}❌ Could not clear port 8888{RESET}")
        return
